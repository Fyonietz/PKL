...
json MySQLPool::db_select_prep(MYSQL* conn, const std::string& query, 
                                std::function<void(PreparedStatement*)> bind_params) {
    PreparedStatement* stmt = nullptr;
    try {
        stmt = new PreparedStatement(conn, query);
        
        // If bind function provided, call it to bind parameters
        if (bind_params) {
            bind_params(stmt);
        }
        ...
        // Get results
        json results = stmt->get_results();
        delete stmt;
        return results;
        
    } catch (const std::exception& e) {
        if (stmt) delete stmt;
        std::cerr << "db_select_prep error: " << e.what() << std::endl;
        return json::array();
    }
}
