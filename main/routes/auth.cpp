...
std::optional<nlohmann::json> CheckAuthToken(struct mg_connection *conn,
  ...
  try {
    ...
    mysql_real_escape_string(db_conn.get(), escaped_token, token.c_str(),
                             token.length());
...
std::string GetAuthToken(struct mg_connection *connection) {
  ...
  while (token_start < auth_str.length() &&
         std::isspace(auth_str[token_start])) {
    token_start++;
  }
